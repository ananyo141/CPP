// A clone of Chapter 12 ATM program to use STL queue instead of homegrown
// Queue class

#include <iostream>
#include <cstdlib>
#include <ctime>
#include "../../Chapter 12/ATM/atm.hpp"
#include <queue>
...
int main() {
    ...
    for (int i = 0; i < totalMins; i++) {
        ...
        if (ATM::checkCustomer(mins_per_customer)) {
            if (line.size() == max_qsize) turnaways++;
            else {
                accepted++;
                newCustomer.set(i);
